...
void performCommand (uint8_t cmd, int16_t par1, char * keystring, int8_t periodicMouseMovement)
{
  ...
  switch (cmd) {
    ...
    case CMD_MM:
      slotSettings.padMode = par1;
      displayUpdate();
      
#ifdef DEBUG_OUTPUT_FULL
      if (slotSettings.padMode == PADMODE_MOUSE)
        Serial.println("mouse function activated");
      else if (slotSettings.padMode >= PADMODE_JOYSTICK_XY)
...
#endif
      initBlink(6, 15);
      if (slotSettings.padMode == PADMODE_ALTERNATIVE)  slotSettings.padMode = PADMODE_MOUSE;
      else slotSettings.padMode = PADMODE_ALTERNATIVE;
      break;
    case CMD_SR:
      reportRawValues = 1;
      break;
