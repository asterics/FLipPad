/*
   ...
   (sent via serial interface, 115200 baud, using spaces between parameters.  Enter (<cr>, ASCII-code 0x0d) finishes a command)
          ...
          AT                returns "OK"
          AT ID             returns identification string (e.g. "FLipPad V1.0")
          AT BM <uint>      puts button into programming mode (e.g. "AT BM 2" -> next AT-command defines the new function for button 2)
                            for the FLipPad, there are 3 physical buttons and 16 virtual buttons(functions) available:
    ...
    Housekeeping commands:
          ...
          AT RS           resets FLipPad and restores default configuration (deletes EEPROM and restores default Slot "mouse")
    ...
    FLipPad-specific slotSettings and commands:
          ...
          AT GV <uint>    tap threshold time (0=disable)  
          AT RV <uint>    trackpad sensitivity   
          AT GH <uint>    tap-gesture threshold time (0=disable)
          AT RH <uint>    TBD
